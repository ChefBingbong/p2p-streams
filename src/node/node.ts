...
import { setMaxListeners, TypedEventEmitter } from "../eventManager/customEventManager";
...
export class Node extends TypedEventEmitter<NodeEvents> {
	...
	constructor(nodeInfo: { id: string; ip: string; port: number }) {
		...
		this.events.dispatchEvent = (evt: any) => {
			...
			const externalResult = this.dispatchEvent(new CustomProgressEvent(evt.type, { detail: evt.detail }));
